...
namespace JavaHook
{
	...
	uint8_t* Module::pattern_scan_text_section( uint8_t* const pattern_start, const int pattern_size, const uint8_t wildcard ) const
	{
	...
	uint8_t* Module::allocate_nearby_memory( uint8_t* nearby_addr, int size, int access )
	{
		for ( int i = 65536;
			i < 0x7FFFFFFF;
			i += 65536 )
